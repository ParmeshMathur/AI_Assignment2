...
class Connect4Board:
    ...
    def move(self, col, player):
        if self.highest[col]>=6:
            ...
            self.board[(5-self.highest[col])*5 + col]=player
            self.highest[col]+=1
        return 0
    ...
    def isWin(self, col, player):
        ...
        if col>0:
            i=1
            while((i<=col and i<=row) and self.board[(row-i)*5 + col-i]==player):
                together+=1
                i+=1
        if col<4:
            i=1
            while((col+i<=4 and row+i<=5) and self.board[(row+i)*5 + col+i]==player):
                together+=1
                i+=1
        if together>=4:
            return True
        ...
        if col>0:
            i=1
            while((i<=col and row+i<=5) and self.board[(row+i)*5 + col-i]==player):
                together+=1
                i+=1
        if col<4:
            i=1
            while((col+i<=4 and i<=row) and self.board[(row-i)*5 + col+i]==player):
                together+=1
                i+=1
        if together>=4:
            return True
        ...
        if col>0:
            i=1
            while(i<=col and self.board[row*5 + col-i]==player):
                together+=1
                i+=1
        if col<4:
            i=1
            while(col+i<=4 and self.board[row*5 + col+i]==player):
                together+=1
                i+=1
        if together>=4:
            return True
        ...
        if row >=3:
            i=1
            while(row+i<=5 and self.board[(row+i)*5 + col]==player):
                together+=1
                i+=1
        if together==4:
            return True
...
def randoPlay(game):
    mo = randint(0,4)
    while(game.highest[mo] <= 5):
        game.move(mo, 1)

# MAIN
def main():
    gameBoard = Connect4Board()
    currstate = gameBoard.board
    mplayer = MCTS()
